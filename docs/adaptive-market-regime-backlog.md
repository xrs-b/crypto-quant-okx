...
## 2026-03-29 orchestration result digest
- [x] Add a backend-only `orchestration_result_digest` layer focused on recent automatic progression events.
- [x] Normalize recent actions for auto approval, controlled rollout, review completion, rollback escalation, recovery retry/re-entry, and testnet bridge execution.
- [x] Expose digest via dashboard API and calibration-report view for direct machine/operator consumption.
- [x] Extend runtime orchestration summary with embedded result digest metadata.
- [x] Add regression tests for helper + API payload shape.
