...
- 2026-03-29 orchestration execution entrypoint：在既有 `runtime_orchestration_summary / production_rollout_readiness` 之上，再补 `GET /api/backtest/adaptive-rollout-orchestration` 同 `calibration-report?view=adaptive_rollout_orchestration`。低干预调用方而家可以直接运行完整 `execute_adaptive_rollout_orchestration(...)` 主线，并稳定拿到 `passes / gate_status / rollout_executor_applied_count / controlled_rollout_executed_count / auto_approval_executed_count / review_queue_queued_count`，唔再只系消费 summary；呢层更贴近生产级 runtime/agent 真正需要的 first-class orchestration execute entrypoint。
