#!/usr/bin/env python3
"""Advice-only CLI for recent parameter tuning suggestions."""
import argparse
import json
import sys
...
from analytics.parameter_tuning_advice import (  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Generate advice-only tuning suggestions from recent outcome attribution samples.'
    )
    ...
    parser.add_argument('--json', action='store_true', help='Emit JSON payload instead of human-readable suggestions')
    return parser
...
def main() -> int:
    ...
    if args.json:
        ...
        return 0
    print(format_parameter_tuning_advice(payload))
    return 0
